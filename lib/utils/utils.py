...
def sample_mask(shape, p=0.002, p_noise=0., max_seq=1, min_seq=1, rng=None):
    print("p",p) #0.0015:(block) #0. :(point)
    print("max_seq",max_seq) #48
    print("min_seq",min_seq) #12
    if rng is None:
    ...
    mask = rand(shape) < p #p:p_fault가 높을 수록 true가 많음
    print("mask0",mask) #(104858, 7)
    for col in range(mask.shape[1]):
        ...
        if not len(idxs):
            continue
        fault_len = min_seq
        print("fault_len1",fault_len) #12
        if max_seq > min_seq: 
            aa = int(randint(max_seq - min_seq)) #0~36
            print("aa",aa) #26
            fault_len = fault_len + aa
        print("fault_len2",fault_len) #38
        idxs_ext = np.concatenate([np.arange(i, i + fault_len) for i in idxs])
        print("idxs_ext",len(idxs_ext))
        idxs = np.unique(idxs_ext)
        idxs = np.clip(idxs, 0, shape[0] - 1)
        print("idxs",idxs)
        print("col",col) #0~6
        mask[idxs, col] = True
    print("p_noise",p_noise) #0.05
    print("mask1",mask.shape,mask) #(104858, 7)
    a = rand(mask.shape) < p_noise #p_noise 값이 커질 수록  True가 많음(마스킹이 적어짐)
    print("rand(mask.shape) < p_noise",a.shape,a) #(104858, 7)
    mask = mask | (a)
    print("mask2",mask)
