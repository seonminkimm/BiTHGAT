...
class SpatialDecoder(nn.Module):
    ...
    def forward(self, x, m, h, u, adj, cached_support=False):
        ...
        out = self.graph_conv(x_in, adj) # MPNN
...
class HGAT(nn.Module):
    ...
    def forward(self, x, adj, mask=None, u=None, h=None, cached_support=False):
        ...
        *_, steps = x.size()
        ...
        # Temporal conv
        imputations = []
        # predictions, states, representations = [], [], []
        ...
        for step in range(steps):
            ...
            x_s = torch.where(m_s, x_s, x_s) # masking
            
            '''
            h_s = h[-1] # torch.Size([32, 64, 36])
            ...
            # first_stage -> first imputation
            # torch.where (조건, data, data2) => mask
            #x_s = torch.where(m_s, x_s, xs_hat_1)
            '''
            
            # prepare inputs
            # retrieve maximum information from neighbors
            # 11.17) return xs_hat_2, repr_s --> xs_hat_2
            xs_hat_2 = self.spatial_decoder(x=x_s, m=m_s, h=h_s, u=u_s, adj=adj,
...
class BiHGAT(nn.Module):
    def __init__(self,
                 ...
                 layer_norm=False,
                 merge='mlp'):
        super(BiHGAT, self).__init__()

        self.fwd_hgat = HGAT(input_size=input_size,
        ...
        self.bwd_hgat = HGAT(input_size=input_size,
                            hidden_size=hidden_size,
                            n_layers=n_layers,
                            dropout=dropout,
                            n_nodes=n_nodes,
    ...
    def forward(self, x, adj, mask=None, u=None, cached_support=False):
        # self.supp : 양방향 adjacency matric
        ...
        # Forward
        # fwd_out, fwd_pred, fwd_repr, _ --> fwd_out
        fwd_out = self.fwd_hgat(x, supp, mask=mask, u=u, cached_support=cached_support)
        # Backward
        rev_x, rev_mask, rev_u = [reverse_tensor(tens) for tens in (x, mask, u)]
        bwd_res = self.bwd_hgat(rev_x, supp, mask=rev_mask, u=rev_u, cached_support=cached_support)
        bwd_out = [reverse_tensor(res) for res in bwd_res]
